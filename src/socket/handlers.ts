...
function emitAutoReveal(io: Server, session: InternalSession): void {
  const result = sessionStore.maybeAutoReveal(session);
  if (result) io.to(session.id).emit('votes_revealed', result);
}

function restoreResult(socket: Socket, session: InternalSession): void {
  if (session.status === 'revealed' && session.revealed_result) {
    socket.emit('votes_revealed', session.revealed_result);
  }
}
...
export function registerHandlers(io: Server, socket: Socket): void {
  ...
  socket.on(
    ...
    (data: {
      ...
      const mode: SessionMode =
        data.session_mode === 'free' ? 'free' : 'stories';

      if (!name || !display_name || !scale) {
        emitError(
          socket,
          'INVALID_INPUT',
          'name, display_name, and a valid voting_scale_id are required',
        );
        return;
      }
    ...
    (data: {
      ...
      if (!session) {
        if (!rateLimiter.isAllowed(ip)) {
          emitError(
            socket,
            'RATE_LIMITED',
            'Too many failed join attempts. Try again in a minute.',
          );
          return;
        }
      ...
      if (data.participant_id) {
        ...
        if (existing) {
          ...
          restoreResult(socket, session);
          socket.to(session.id).emit('participant_reconnected', {
      ...
      const role: ParticipantRole =
        data.role === 'observer' ? 'observer' : 'team_member';
      const participant_id = generateId();
  ...
  socket.on('cast_vote', (data: { card_value?: string }) => {
    ...
    if (!lookup) {
      emitError(socket, 'NOT_IN_SESSION', 'Not in a session');
      return;
    }
    ...
    if (
      !data.card_value ||
      !session.voting_scale.cards.includes(data.card_value)
    ) {
      emitError(
        socket,
        'INVALID_CARD',
        "card_value must be a valid card for this session's scale",
      );
      return;
    }
    ...
    if (!ok) {
      emitError(
        socket,
        'INVALID_STATE',
        'Cannot vote in the current session state',
      );
      return;
    }

    io.to(session.id).emit('vote_cast', { participant_id: participant.id });
    emitAutoReveal(io, session);
  });

  socket.on('set_auto_reveal', (data: unknown) => {
    const lookup = sessionStore.getParticipantBySocket(socket.id);
    if (!lookup) {
      emitError(socket, 'NOT_IN_SESSION', 'Not in a session');
      return;
    ...
    if (participant.role !== 'moderator') {
      emitError(
        socket,
        'FORBIDDEN',
        'Only the Moderator can change auto-reveal',
      );
      return;
    }
    if (
      ...
      emitError(socket, 'INVALID_INPUT', 'enabled must be a boolean');
      return;
    }
    sessionStore.setAutoReveal(session, data.enabled);
    io.to(session.id).emit('auto_reveal_changed', { enabled: data.enabled });
    emitAutoReveal(io, session);
  });
  ...
  socket.on('reveal_votes', () => {
    ...
    if (!result) {
      emitError(
        socket,
        'INVALID_STATE',
        'Cannot reveal votes in the current session state',
      );
      return;
    }
  ...
  socket.on('add_story', (data: { title?: string; description?: string }) => {
    ...
    if (!title) {
      emitError(socket, 'INVALID_INPUT', 'Story title is required');
      return;
    }
  ...
  socket.on('set_active_story', (data: { story_id?: string }) => {
    ...
    if (participant.role !== 'moderator') {
      emitError(
        socket,
        'FORBIDDEN',
        'Only the Moderator can set the active story',
      );
      return;
    }
    ...
    if (!ok) {
      emitError(socket, 'NOT_FOUND', 'Story not found');
      return;
    }
  ...
  socket.on(
    'finalize_story',
    (data: { story_id?: string; final_estimate?: string }) => {
      const lookup = sessionStore.getParticipantBySocket(socket.id);
      if (!lookup) {
        emitError(socket, 'NOT_IN_SESSION', 'Not in a session');
        return;
      ...
      if (participant.role !== 'moderator') {
        emitError(
          socket,
          'FORBIDDEN',
          'Only the Moderator can finalize stories',
        );
        return;
      }
      ...
      if (!estimate) {
        emitError(socket, 'INVALID_INPUT', 'final_estimate is required');
        return;
      }

      const story = sessionStore.finalizeStory(
        session,
        data.story_id ?? '',
        estimate,
      );
  ...
  socket.on('transfer_sm', (data: { new_sm_id?: string }) => {
    ...
    if (participant.role !== 'moderator') {
      emitError(
        socket,
        'FORBIDDEN',
        'Only the Moderator can transfer the role',
      );
      return;
    }
    ...
    if (!ok) {
      emitError(socket, 'NOT_FOUND', 'Participant not found');
      return;
    }
  ...
  // ---------------------------------------------------------------------------
  // remove_participant  (Moderator only)
  // Payload: { participant_id }
  // ---------------------------------------------------------------------------
  socket.on('remove_participant', (data: { participant_id?: string }) => {
    const lookup = sessionStore.getParticipantBySocket(socket.id);
    if (!lookup) {
      emitError(socket, 'NOT_IN_SESSION', 'Not in a session');
      return;
    ...
    const target_id = data.participant_id ?? '';
    if (!target_id || target_id === participant.id) {
      emitError(socket, 'INVALID_INPUT', 'Cannot remove yourself');
      return;
    }

    const removed = sessionStore.removeParticipant(session, target_id);
    if (!removed) {
      emitError(socket, 'NOT_FOUND', 'Participant not found');
      return;
    }

    if (removed.socket_id) {
      const targetSocket = io.sockets.sockets.get(removed.socket_id);
      if (targetSocket) {
        targetSocket.emit('removed_from_session', {});
        targetSocket.leave(session.id);
      }
    }

    io.to(session.id).emit('participant_removed', {
  ...
  socket.on('disconnect', () => {
    ...
    io.to(mapping.session_id).emit('participant_left', {
    ...
    setTimeout(() => {
      ...
      io.to(mapping.session_id).emit('sm_transferred', {
